...
**Гони на СВОЕЙ базе и сноси схему перед каждым прогоном.** Общая `prov_fields` уже испортила одну батарею: мутация, поменявшая определение колонки, **осталась в базе навсегда** — идемпотентная миграция существующую колонку не правит. Пять следующих мутаций краснели от чужого следа, а контрольный прогон после серии оказался красным на чистом коде. Заведи свою:

```bash
ssh dv@85.192.61.231 'sudo -u postgres psql -qc "DROP DATABASE IF EXISTS rent_<твоя-задача>"; \
  sudo -u postgres psql -qc "CREATE DATABASE rent_<твоя-задача> OWNER dv"'
```

и сноси её между прогонами. **Снос в коде не закодирован** — это шаг твоего прогона, а не свойство сьюта.

**Идемпотентность проверяется на СПИСКЕ, а не на файле.** Модуль накатывает все миграции при каждом старте API. Отдельный файл может быть безупречен, а список — сломан: ранняя миграция навешивает свой словарь допустимых значений на данные, которых тогда не существовало, и падает. Схема при этом остаётся исправной (её доводит поздняя миграция), отказ уходит в лог, и ранняя миграция **становится мёртвой молча и навсегда**. Так уже случилось со второй миграцией и статусом сна. Правило: значение, дописанное в **именованный** словарь поздней миграцией, дописывается и во все ранние, где тот же словарь объявлен заново.
...
> **СДЕЛАНА, и SQL ниже содержит ПЯТЬ дефектов. Рецепт не повторять — читать реализацию в `rent.service.ts`.**
>
> **1. `GREATEST(0, …)` — не защита, а сам дефект.** Я назвал его смягчением редкой гонки. Измерено на живом PostgreSQL: баланс 60 000, параллельная правка забирает 20 000 → владельцу остаётся ноль (взяли 40 000 из нужных 50 000) **и месяц засчитан**. Это ровно тот частичный расход, от которого этот же раздел уходит двумя абзацами выше. `GREATEST` молча превращает недостачу в частичное списание — то есть прячет то, ради чего написан. Лечится чтением баланса **под замком** (`FOR UPDATE` в условии занятия периода) плюс повторной сверкой достатка в самом списании.
>
> **2. Сдвиг `paid_until + interval '1 month'` копит долг.** Просроченный на три месяца остаётся с датой в прошлом и списывается снова на каждом обороте: измерено 200 000 → 150 000 → 100 000 при сроке в августе. Спека копящийся долг запрещает. Верно: просрочка меньше месяца — якорь даты сохраняется (иначе он уезжает вперёд на время запаздывания сборщика, до двух недель бесплатного хостинга в год на продукт); просрочка большая — месяц от `now()`.
>
> **3. Аренда шла мимо учёта токенов.** Спека требует общего учёта. Без записи в `token_transactions` владелец видит исчезнувшие 50 000 без объяснения, а админские отчёты аренду не видят вовсе.
>
> **4. `rowCount` как признак «списалось»** скрывает расхождение «период занят, а денег не взяли» — бесплатный месяц.
>
> **5. Тесты были слепы к решению владельца:** все сценарии на `running`/`sleeping`, мутация «платит только `running`» оставалась зелёной — при том что на проде четыре продукта из шести в `degraded`.
>
> **Урок про измерение, который стоит унести дальше:** мутация «сначала списать, потом занять период» в первом прогоне ловилась **только проверкой формы запроса**. Сценарий с двумя одновременными сборщиками был зелёным — двойное списание там ловилось по воле планировщика, а не по устройству теста. Понадобился сценарий, где оба гарантированно встают на занятую строку (ожидание по `pg_stat_activity`, а не пауза). Подозрительно зелёный тест — это повод выяснить, почему он зелёный, а не повод радоваться.
