...
**Tech Stack:** NestJS 10, PostgreSQL, Redis (для in-memory rate-limit и pub/sub re-check), `grammy` (Telegram bot lib), Anthropic SDK (Claude Sonnet для ответов, Haiku 4.5 для smart-gate), OpenAI SDK (Whisper STT + TTS), React 18, Tailwind.
...
CREATE TABLE IF NOT EXISTS tg_user_identities (
  linkeon_user_id  uuid PRIMARY KEY,
...
// внутри класса добавить поле и провайдер
...
/**
 * Вызов Claude и возврат текста ответа + cost USD.
 */
...
${systemPrompt}`;
  ...
  // Compute USD cost — Sonnet 4.6 pricing: $3/$15 per 1M (input/output)
  const inputTok = resp.usage.input_tokens ?? 0;
  const outputTok = resp.usage.output_tokens ?? 0;
  const costUsd = (inputTok * 3 + outputTok * 15) / 1_000_000;
...
В импорты файла добавить `AgentsService`.

- [ ] **Step 2: Импортировать AgentsModule в TgBotModule**
...
```bash
git commit -am "feat(tg-bot): LLM-вызов через Anthropic SDK + persist истории"
```
