...
> **26.09.2026, после выката:** владелец снял ленту сессий («не нужно подробностей,
> нужно видеть статистику»). `CallSessionsFeed` удалён, номер-кнопка из
> `CallSessionItem` — тоже; задачи плана про ленту выполнены и затем отменены.
> Подробности — в обновлении спеки.
...
- **Task 9:**
  - идущая сессия (`live`) перечитывает расшифровку при каждом раскрытии;
  - по ревью: `block` больше не отменяет `line-clamp-2`;
  - расшифровка кешируется по версии сессии (`status:user_turns`), строка, ставшая нераскрываемой, закрывается;
  - ошибка загрузки видна отдельно (`admin.calls.transcriptFailed`) и не кешируется;
  - ответ устаревшего запроса не затирает свежий;
  - битые реплики отбрасываются;
  - раскладка списания видна и в раскрытой строке;
  - длительность — `formatClock` в `callsFormat.ts` (с часами);
  - 404 с JSON-телом закреплён тестом.
- **Task 10 (по ревью):**
  - потолок 500 решается по показанному ответу (`pages`, `data.asked`), так что после сбоя последней порции есть повтор, а сервер с потолком ниже тоже считается потолком;
  - проверяется форма ответа;
  - кнопка во время загрузки не теряет фокус;
  - добавлены тест «Обновить» и число на кнопке.
- **Task 11 (по ревью):**
  - ответы таблицы защищены от гонки;
  - кнопки площадок строятся только по ответу «Встреч»;
  - выбранная площадка без сессий остаётся кнопкой с нулём;
  - подписи — по показанному ответу;
  - у всех кнопок фильтров `aria-pressed`, у групп — `role="group"`;
  - добавлен тест «Обновить».
- **Финальное ревью:**
  - `formatTokens` терпит нечисло — новый фронт на старом бэкенде больше не роняет приложение в карточке;
  - `interrupted` подписан «обрыв»;
  - карточка показывает ошибку загрузки и «50+» при полной порции.
...
Ожидается (итог после всех правок ревью, включая финальное): `Test Files 65 passed (65)`, `Tests 824 passed (824)`. Замер 26.09.2026 до финальных правок — 64 файла и 819 тестов. Разбивка ниже — исходная, до правок ревью Task 9–11:
- `callsLocales` — 5;
- `callProviders` — 4;
