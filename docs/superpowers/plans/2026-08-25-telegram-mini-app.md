...
## Мелкие хвосты, подобрать в финальном проходе

Найдены ревью по ходу работы, ни один не блокирует свою задачу.

- **`tailwind.config.js`**: в `content` нет `tma.html` — только `index.html` перечислен поимённо. Сейчас безвредно (классов в разметке `tma.html` нет), но однострочная правка снимает мину на будущее.
- **`tma.controller.ts`**: у вызовов `touchIdentity` нет своей метки `stage`, поэтому их падение залогируется под меткой предыдущей операции. Не уводит в чужую ветку, но метка врёт.
- **`AuthController` — десять зависимостей в конструкторе.** Именно эта хрупкость и породила регрессию в Task 4: добавление параметра сломало компиляцию чужого теста. Кандидат на вынос ручек привязки идентичностей в отдельный контроллер — но это отдельная задача, не хвост.

## Что проверено мутациями, а не только зелёным прогоном

Практика, которая себя оправдала: после каждого зелёного прогона ломать
проверяемое поведение нарочно и убеждаться, что тест краснеет. Так поймано:

- пинящий тест Task 2 грепал весь файл и не замечал урезанный `CHECK` —
  имена провайдеров случайно упоминались в комментарии-шапке;
- обвязка `reauthHandler` в Task 7 не была покрыта вовсе: её можно было
  удалить целиком, и набор оставался зелёным;
- зависание очереди запросов при бросающем `reauthHandler` — воспроизведено
  сносом `try/catch`, а не выведено рассуждением.

Во всех трёх случаях покрытие выглядело достаточным, а проверки не было.
