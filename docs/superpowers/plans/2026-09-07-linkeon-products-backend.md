...
**Мок `pg.query` не исполняет SQL — он его игнорирует.** Из этого следует правило: если поведение задаётся текстом запроса (фильтр по владельцу, `WHERE` по токену, блокировка строки, порог по времени), тест обязан утверждать про текст SQL, а не только про переданные параметры. Проверка одних параметров фиксирует форму вызова: убери условие из `WHERE`, оставив параметр на месте, — и такой тест останется зелёным, назвавшись при этом защитой от утечки. На этом проекте уже было пять ложно-зелёных проверок подряд в i18n; здесь тот же механизм.

Способ убедиться, что тест не ложно-зелёный, один: **сломать проверяемое поведение нарочно и увидеть красный.** Зелёный прогон сам по себе не доказывает ничего.
