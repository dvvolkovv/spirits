...
## Как проверять — рабочие команды

В воркдереве `pnpm` спотыкается на `ERR_PNPM_IGNORED_BUILDS` (esbuild). Рабочие варианты:

```bash
pnpm --config.verify-deps-before-run=false typecheck   # ЕДИНСТВЕННАЯ настоящая проверка типов
pnpm vitest run                                         # юнит-тесты
node_modules/.bin/vite build                            # сборка
pnpm check-locales                                      # паритет ключей локалей
```

Голый `tsc --noEmit` — **пустышка**: корневой `tsconfig.json` это заглушка с `"files": []`, команда всегда возвращает 0. `pnpm build` (vite) типы не проверяет, а стирает.

В проекте **48 унаследованных ошибок типов** (`ChatInterface`, `pushClient`, `VideoJobCard` и др.). Ориентир — не «ноль», а «мои файлы не добавили ни одной»: сравнивай вывод до и после правок побайтово, а не по счётчику.

Если появился артефакт `pnpm-workspace.yaml` — удали, он не нужен.

### Две ловушки, найденные по ходу

**TypeScript не проверяет дефолты деструктуризации против union.** Опечатка в `variant = 'defualt'` проходит молча и роняет компонент в ветку по умолчанию — проверено намеренной подменой. Передача пропса снаружи (`variant="pill"`) проверяется нормально. Вывод: дефолты сверяй глазами, на компилятор тут не рассчитывай.

**`className` в `LanguageSelect` ведёт себя по-разному в двух ветках.** В `variant="pill"` он ДОБАВЛЯЕТСЯ к базовым классам, в ветке по умолчанию — ЗАМЕЩАЕТ их. Значит передать пилюле `className` с своей рамкой или фоном не выйдет: они сложатся с базовыми. Задача 11 это учитывает — там пилюля позиционируется обёрткой снаружи, а не своим `className`.
...
```tsx
  return (
    {/* Своих вертикальных отступов НЕТ: интервалы задаёт space-y-3 на форме,
        иначе между полем и кнопкой получается двойной зазор. */}
    <div>
...
> Серая карточка `bg-gray-50 rounded-lg p-4` убрана: блок больше не самостоятельная секция наверху, а строка внутри формы. Вертикальных отступов у него нет — интервалы задаёт `space-y-3` на самой форме (`EmailLoginPane`, `PhoneInput`), иначе зазор удваивается.
