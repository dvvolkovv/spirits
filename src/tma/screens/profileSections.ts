 ...
 * `desiresRich: [{ name, gloss, aliases, support }]` — её отдаёт граф.
 * `gloss` — обоснование, то самое, что веб показывает под пунктом; часто
 * пустая строка. `support` — сколько раз пункт встретился в разговорах,
 * веб рисует его как «×N». Веб-профиль показывает богатую форму, если она
 * есть, и падает на простую иначе; здесь то же самое, иначе один и тот же
 * профиль выглядел бы в двух местах по-разному.
 *
 * Поле называется именно `gloss` — проверено на живом профиле прода
 * 07.09.2026. Первая редакция этого модуля искала `description`, которого в
 * данных нет вовсе, и обоснования не показывались.
 *
...
export interface SectionItem {
  name: string;
  /** Обоснование из графа. Пустую строку считаем отсутствием. */
  gloss?: string;
  /** Сколько раз пункт встретился в разговорах. */
  support?: number;
}
    ...
    return rich
      .map((r: any) => {
        ...
        return typeof r?.name === 'string'
          ? {
              name: r.name,
              gloss: typeof r?.gloss === 'string' && r.gloss.trim() ? r.gloss : undefined,
              support: typeof r?.support === 'number' && r.support > 1 ? r.support : undefined,
            }
          : { name: '' };
      })
      .filter((i) => i.name);
