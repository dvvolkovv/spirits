...
const AdminRetentionView: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const r = await apiClient.post('/webhook/admin/retention', { action: 'preview' });
  ...
  const send = async (phones?: string[], resend = false) => {
    ...
    setError(null);
    try {
      const r = await apiClient.post('/webhook/admin/retention', { action: 'send', confirm: true, phones, resend });
