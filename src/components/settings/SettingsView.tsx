...
const SettingsView: React.FC = () => {
  ...
  const togglePush = async () => {
    ...
    try {
      if (pushOn) {
        ...
        setPushOn(ok);
        if (!ok) setPushMsg(t('settings.push.enable_error'));
      }
    } finally {
      setPushBusy(false);
    }
  ...
  const testPush = async () => {
    ...
    try {
      ...
      setPushMsg(ok ? t('settings.push.test_success') : t('settings.push.test_error'));
    } finally {
      setPushBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm">
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="pr-4">
                <h3 className="text-sm font-medium text-gray-900">{t('settings.push.label')}</h3>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900 flex items-center">
              ...
              {t('settings.contact_visibility.title')}
            </h2>
          </div>
          <div className="p-6 space-y-3">
            <p className="text-sm text-gray-600">
              {t('settings.contact_visibility.desc')}
            </p>
            {([
              { v: 'public',    title: t('settings.contact_visibility.public_title'),  desc: t('settings.contact_visibility.public_desc') },
              { v: 'matchOnly', title: t('settings.contact_visibility.match_title'),   desc: t('settings.contact_visibility.match_desc') },
              { v: 'private',   title: t('settings.contact_visibility.private_title'), desc: t('settings.contact_visibility.private_desc') },
        ...
        <p className="mt-8 text-center text-xs text-gray-400">
          {t('settings.footer_build', { build: APP_BUILD })}
        </p>
