...
describe('досылка не проигрывает гонку истории', () => {
  it('перед отправкой подтягивает историю и ЖДЁТ её', () => {
    // Замер на проде 10.09.2026: удалённый ответ сохранён в 06:11:05.6,
    // досылка ушла в 06:11:07.3 — через 1.7 секунды. Поллинг ходит раз в 8с и
    // в это окно не попал. Отправив первыми, мы делаем своё локальное
    // сообщение самым свежим, и selectNewPolledMessages отбрасывает удалённый
    // ответ как более старый — навсегда, до перезагрузки страницы.
    const flush = SRC.slice(SRC.indexOf('Досылка очереди'));
    const body = flush.slice(0, flush.indexOf('}, ['));
    expect(body).toMatch(/void pullHistoryOnce\(assistantIdForFlush\)\s*\n\s*\.then\(\(\) => sendMessageText\(text\)\)/);
  });

  it('подтягивание истории — общая функция с поллингом, а не копия', () => {
    // Копия разъехалась бы с правилами дедупа в historyMerge.
    expect(SRC).toMatch(/const pullHistoryOnce = async \(/);
    expect(SRC).toMatch(/const poll = \(\) => pullHistoryOnce\(assistantId, \(\) => cancelled\);/);
  });
});
