...
export async function refreshWidget(): Promise<void> {
  ...
  try {
    ...
    if (bridge.setShortcuts) {
      ...
      items.push({
        id: 'resume',
        shortLabel: i18n.t('widget.resume'),
        longLabel: d.assistantName
          ? i18n.t('widget.resume_with', { name: d.assistantName })
          : i18n.t('widget.resume'),
      ...
      if (recents[0]) {
        items.push({
          ...
          longLabel: i18n.t('widget.open_named', { name: recents[0].name }),
          path: `/chat?assistant=${encodeURIComponent(recents[0].id)}&src=shortcut`,
        });
      }
      items.push({ id: 'image', shortLabel: i18n.t('widget.image'), longLabel: i18n.t('widget.image_long'), path: '/image-gen?src=shortcut' });
      items.push({ id: 'voice', shortLabel: i18n.t('widget.voice'), longLabel: i18n.t('widget.voice_long'), path: '/chat?voice=1&resume=1&src=shortcut' });
      try {
